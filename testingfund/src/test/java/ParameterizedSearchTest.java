...
public class ParameterizedSearchTest {
    ...
    @Test
    @Parameters("userInputSearch")
    public void checkSearchFunc(String userInputSearch){
        mainPage.search(userInputSearch);
        List<WebElement> search_results = mainPage.getResults();
        for (WebElement element : search_results) {
            assert(element.getText().toLowerCase().contains(userInputSearch));
        }
    }

    @Test
    @Parameters("userInputCourse")
    public void checkSearchFuncCourses(String userInputCourse){
        coursesPage = mainPage.goToCourses();
        coursesPage.searchCourses(userInputCourse);
        List<WebElement> search_results_courses = coursesPage.getCoursesResults();
        for (WebElement element : search_results_courses) {
            assert(element.getText().toLowerCase().contains(userInputCourse));
        }
    }
